...
function ProjectionChart({ projection }: { projection: { label: string; total: number }[] }) {
  ...
  return <Card><div className="flex flex-wrap items-end justify-between gap-2"><div><h2 className="font-bold">Proyección de pagos</h2><p className="mt-1 text-sm text-slate-500">Estimación de cargos recurrentes para los próximos 12 meses.</p></div><span className="text-xs text-slate-500">Solo moneda principal</span></div><div className="mt-6 grid grid-cols-12 items-end gap-1.5 sm:gap-2">{projection.map(item => <div key={item.label} className="grid gap-2 text-center"><span className="text-[10px] text-slate-600">{item.total.toLocaleString("es-MX", { maximumFractionDigits: 0 })}</span><div className="h-32 rounded-t-lg bg-slate-100 p-1"><div className="h-full rounded-md bg-indigo-500" style={{ height: `${Math.max(item.total ? 8 : 0, item.total / max * 100)}%`, marginTop: `${100 - Math.max(item.total ? 8 : 0, item.total / max * 100)}%` }} /></div><span className="text-[10px] capitalize text-slate-500">{item.label}</span></div>)}</div><p className="mt-4 text-sm font-semibold text-slate-700">Total anual estimado: {projection.reduce((sum, item) => sum + item.total, 0).toLocaleString("es-MX", { maximumFractionDigits: 2 })}</p></Card>;
}

function RuleFields({ categories, paymentMethods, currency, initial }: { categories: { id: string; name: string }[]; paymentMethods: { id: string; name: string }[]; currency: string; initial?: { type: string; amount: unknown; currency: string; frequency: string; nextRun: Date; note: string | null; categoryId: string | null; paymentMethodId: string | null; kind?: string; provider?: string | null; chargeDay?: number | null } }) {
  return <><div className="grid gap-3 sm:grid-cols-2"><select name="type" className={inputCls} defaultValue={initial?.type ?? "EXPENSE"}><option value="EXPENSE">Gasto</option><option value="INCOME">Ingreso</option></select><input name="amount" className={inputCls} placeholder="Monto" defaultValue={String(initial?.amount ?? "")} required /></div><div className="grid gap-3 sm:grid-cols-2"><input name="currency" className={inputCls} defaultValue={initial?.currency ?? currency} required /><select name="frequency" className={inputCls} defaultValue={initial?.frequency ?? "MONTHLY"}><option value="WEEKLY">Semanal</option><option value="MONTHLY">Mensual</option><option value="YEARLY">Anual</option></select></div><div className="grid gap-3 sm:grid-cols-2"><select name="kind" className={inputCls} defaultValue={initial?.kind ?? "SERVICE"}><option value="SERVICE">Servicio</option><option value="MEMBERSHIP">Membresía</option></select><input name="provider" className={inputCls} placeholder="Proveedor" defaultValue={initial?.provider ?? ""} /></div><div className="grid gap-3 sm:grid-cols-2"><input name="chargeDay" type="number" min="1" max="31" className={inputCls} placeholder="Día de cobro" defaultValue={initial?.chargeDay ?? ""} /><input name="nextRun" type="date" className={inputCls} defaultValue={toInputDate(initial?.nextRun ?? new Date())} required /></div><div className="grid gap-3 sm:grid-cols-2"><select name="categoryId" className={inputCls} defaultValue={initial?.categoryId ?? ""}><option value="">Sin categoría</option>{categories.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select><select name="paymentMethodId" className={inputCls} defaultValue={initial?.paymentMethodId ?? ""}><option value="">Sin medio de pago</option>{paymentMethods.map(method => <option key={method.id} value={method.id}>{method.name}</option>)}</select></div><input name="note" className={inputCls} placeholder="Aviso (opcional)" defaultValue={initial?.note ?? ""} /></>;
}
