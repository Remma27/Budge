  ...
  return (
    <div className="grid gap-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <div className="flex flex-wrap gap-3 text-sm"><Link className="underline" href={`/api/export?format=csv&mes=${mes}${workspaceId ? `&workspaceId=${workspaceId}` : ""}${moneda ? `&moneda=${moneda}` : ""}${categoria ? `&categoria=${categoria}` : ""}${tipo ? `&tipo=${tipo}` : ""}${q ? `&q=${encodeURIComponent(q)}` : ""}`}>Exportar CSV filtrado</Link><Link className="underline" href={`/api/export?format=backup${workspaceId ? `&workspaceId=${workspaceId}` : ""}`}>Descargar respaldo completo</Link></div>
