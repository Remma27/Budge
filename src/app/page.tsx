  ...
  return (
    <div className="grid gap-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
       ...
       {budgets.length > 0 && <Card><h2 className="mb-3 font-bold">Presupuesto del mes</h2><div className="grid gap-3 text-sm">{budgets.map(b => { const spent = txs.filter(t => t.type === "EXPENSE" && t.categoryId === b.categoryId && t.currency === b.currency).reduce((sum, t) => sum + Number(t.amount), 0); const income = txs.filter(t => t.type === "INCOME" && t.currency === b.currency).reduce((sum, t) => sum + Number(t.amount), 0); const effectiveAmount = b.mode === "PERCENTAGE" ? income * Number(b.percentage) / 100 : Number(b.amount); const pct = effectiveAmount ? Math.min(100, Math.round(spent / effectiveAmount * 100)) : 0; return <div key={b.id}><div className="flex justify-between"><span>{b.category.name} · {b.currency}</span><span className="font-medium">{formatMoney(spent, b.currency)} / {formatMoney(effectiveAmount, b.currency)}</span></div>{b.mode === "PERCENTAGE" && <p className="text-xs text-zinc-500">{String(b.percentage)}% de ingresos del periodo ({formatMoney(income, b.currency)})</p>}<div className="mt-1 h-2 rounded bg-zinc-200"><div className={`h-2 rounded ${pct >= 100 ? "bg-red-600" : pct >= 80 ? "bg-amber-500" : "bg-green-600"}`} style={{ width: `${pct}%` }} /></div>{pct >= 80 && <p className={`mt-1 text-xs ${pct >= 100 ? "text-red-600" : "text-amber-600"}`}>{pct >= 100 ? "Presupuesto excedido" : "Alerta: alcanzaste 80%"}</p>}</div>})}</div></Card>}
